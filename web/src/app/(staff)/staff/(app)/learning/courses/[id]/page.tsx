...
import { CourseWorkspaceNav } from "@/components/staff/CourseWorkspaceNav";
import { CourseDangerZone } from "@/components/staff/CourseDangerZone";
import {
  StaffEmpty,
  StaffMetricGrid,
  StaffPanel,
...
export default async function StaffCourseWorkspacePage({
  params,
  searchParams,
}: {
  ...
  const focusMember = typeof query.member === "string" ? query.member : undefined;

  const courseId = Number(id);
  if (!Number.isFinite(courseId)) notFound();

  const selected = await payload
    .findByID({ collection: "lms-courses", id: courseId, depth: 0, ...access })
    .catch(() => null);
  ...
  const base = `/staff/learning/courses/${courseID}`;
  ...
  return (
    <div className={cn("space-y-6", staffOpsChrome)}>
      <div className="rise">
        <Link
          href="/staff/learning"
          className="inline-flex items-center gap-2 text-sm text-text-3 transition-colors hover:text-text-1"
        >
        ...
        <div className="mt-4 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Chip tone={selected.status === "published" ? "ai" : selected.status === "draft" ? "warn" : "neutral"}>
                {String(selected.status)}
              </Chip>
              {!readiness.ready && selected.status !== "published" ? (
                <Chip tone="warn">{readiness.missing.length} to fix before publishing</Chip>
              ) : null}
            </div>
            <h1 className="mt-3 font-display text-2xl text-text-1 sm:text-3xl">{selected.title}</h1>
            <p className="mt-1 font-mono text-xs text-text-3">{selected.programKey}</p>
            {selected.summary ? (
              <p className="mt-2 max-w-2xl text-sm leading-relaxed text-text-2">{selected.summary}</p>
            ) : null}
          </div>
          <div className="flex shrink-0 flex-wrap gap-2">
            {selected.status === "published" ? (
              <Button
                href={`/app/learning/courses/${selected.slug}`}
                target="_blank"
                rel="noreferrer"
                variant="secondary"
              >
            ...
            <Button href={`${base}?tab=settings`} variant="secondary">
              Settings
            </Button>
          </div>
        </div>
      ...
      <StaffMetricGrid
        items={[
          isCohort
            ? { label: "Live sessions", value: sessions.totalDocs, tone: "ai" as const }
            : { label: "Modules", value: modules.totalDocs, tone: "accent" as const },
          { label: "Lessons", value: lessons.totalDocs, tone: "accent" as const },
          { label: "Learners", value: enrollments.totalDocs, tone: "accent" as const },
          { label: "Completions", value: completed, tone: "ai" as const },
        ]}
      />
      ...
      <CourseWorkspaceNav
        ...
        cohort={isCohort}
        needsAttention={!readiness.ready}
      />

      {activeTab === "overview" ? (
        <div className="grid gap-4 lg:grid-cols-2">
          <StaffPanel>
            <p className="eyebrow text-accent">Readiness</p>
            <h3 className="mt-2 font-display text-xl text-text-1">Publication checklist</h3>
            <ul className="mt-4 space-y-2">
              {readiness.checks.map(({ label, ready }) => (
                <li key={label} className={cn("flex items-center gap-2 text-sm", ready ? "text-ai" : "text-text-3")}>
            ...
            {!readiness.ready ? (
              <p className="mt-4 text-sm text-text-2">
                ...
                <Link href={`${base}?tab=settings`} className="text-accent hover:underline">
                  Settings
                </Link>
                .
              </p>
          ...
          <StaffPanel>
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="eyebrow text-accent">Course details</p>
                <h3 className="mt-2 font-display text-xl text-text-1">At a glance</h3>
              </div>
              <Link href={`${base}?tab=settings`} className="text-xs text-accent hover:underline">
                Edit in Settings
              </Link>
            </div>
            <dl className="mt-4 space-y-3 text-sm">
              <div className="flex justify-between gap-4 border-b border-edge-subtle pb-2">
                <dt className="text-text-3">Instructor</dt>
                <dd className="text-right text-text-1">{selected.instructor || "Not set — open Settings"}</dd>
              </div>
              <div className="flex justify-between gap-4 border-b border-edge-subtle pb-2">
                <dt className="text-text-3">Category</dt>
                <dd className="text-right text-text-1">{selected.category || "Not set — open Settings"}</dd>
              </div>
              <div className="flex justify-between gap-4 border-b border-edge-subtle pb-2">
                <dt className="text-text-3">Learning outcomes</dt>
              ...
              <div className="flex justify-between gap-4 border-b border-edge-subtle pb-2">
                <dt className="text-text-3">Program key</dt>
                <dd className="text-text-1">{selected.programKey}</dd>
              </div>
              <div className="flex justify-between gap-4 border-b border-edge-subtle pb-2">
                <dt className="text-text-3">Access</dt>
                <dd className="text-text-1">{selected.accessRule}</dd>
              </div>
              <div className="flex justify-between gap-4 border-b border-edge-subtle pb-2">
                <dt className="text-text-3">Level</dt>
                <dd className="text-text-1">{selected.level ?? "Not set"}</dd>
              </div>
              <div className="flex justify-between gap-4">
                <dt className="text-text-3">Estimated time</dt>
                <dd className="text-text-1">{selected.estimatedHours ? `${selected.estimatedHours} hours` : "Not set"}</dd>
              </div>
            </dl>
          </StaffPanel>
        </div>
      ...
      {activeTab === "curriculum" ? (
        <StaffPanel>
          <div className="mb-4">
            <p className="eyebrow text-accent">Curriculum</p>
            <h3 className="mt-2 font-display text-xl text-text-1">Modules and lessons</h3>
          </div>
          {modules.docs.length ? (
            <div className="space-y-4">
              {modules.docs.map((courseModule, moduleIndex) => {
                ...
                return (
                  <article key={courseModule.id} className="rounded-[var(--radius-lg)] border border-edge-subtle bg-inset p-4">
                    <header className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                      <div>
                        <small className="eyebrow text-text-3">Module {moduleIndex + 1}</small>
                        <h4 className="mt-1 text-lg text-text-1">{courseModule.title}</h4>
                        <Link
                    ...
                    <ol className="mt-4 space-y-2">
                      {moduleLessons.map((lesson, lessonIndex) => (
                        <li
                          key={lesson.id}
                          className="flex flex-col gap-2 rounded-[var(--radius-md)] border border-edge-subtle px-3 py-3 sm:flex-row sm:items-center sm:justify-between"
                        >
                          <span>
                            <Link href={`/staff/learning/lessons/${lesson.id}`} className="block text-sm font-semibold text-text-1 hover:text-accent">
                              {lesson.title}
                            </Link>
      ...
      {activeTab === "sessions" && isCohort ? (
        <StaffPanel>
          <div className="mb-4">
            <p className="eyebrow text-accent">Cohort</p>
            <h3 className="mt-2 font-display text-xl text-text-1">Live sessions</h3>
          ...
          {sessions.docs.length ? (
            <div className="space-y-3">
              {sessions.docs.map((session) => {
                ...
                return (
                  <article key={session.id} className="rounded-[var(--radius-lg)] border border-edge-subtle bg-inset p-4">
                    <header className="flex flex-wrap items-start justify-between gap-3">
                      <div>
                        <h4 className="text-lg text-text-1">{session.title}</h4>
                        <small className="text-xs text-text-3">
                      ...
                      {session.joinUrl ? (
                        <a href={String(session.joinUrl)} target="_blank" rel="noreferrer" className="text-xs text-accent hover:underline">
                          Join link
                        </a>
                      ) : null}
                    </header>
                    <details className="mt-3 rounded-[var(--radius-md)] border border-edge-subtle bg-inset p-3">
                      <summary className="cursor-pointer text-sm text-text-2">Take attendance ({roster.length})</summary>
                      <AttendanceRegister
                    ...
                    <details className="mt-2 rounded-[var(--radius-md)] border border-edge-subtle bg-inset p-3">
                      <summary className="cursor-pointer text-sm text-text-2">Edit session</summary>
                      <div className="mt-4">
                        <StaffRecordForm
      ...
      {activeTab === "announcements" && isCohort ? (
        <StaffPanel>
          <div className="mb-4">
            <p className="eyebrow text-accent">Cohort</p>
            <h3 className="mt-2 font-display text-xl text-text-1">Announcements</h3>
            <p className="mt-1 text-sm text-text-2">Broadcast updates to everyone in this cohort. Pinned posts stay at the top of their feed.</p>
          </div>
          ...
          <div className="mt-4 space-y-2">
            {announcements.docs.length ? (
              announcements.docs.map((announcement) => {
                ...
                return (
                  <article key={announcement.id} className="rounded-[var(--radius-lg)] border border-edge-subtle bg-inset p-4">
                    <div className="flex flex-wrap items-center gap-2">
                      {announcement.pinned ? (
                        <span className="rounded-full border border-accent/30 bg-accent-bg px-2 py-0.5 text-[10px] uppercase tracking-wide text-accent">Pinned</span>
                      ) : null}
                      <h4 className="text-base text-text-1">{announcement.title}</h4>
                    ...
                    <p className="mt-2 whitespace-pre-line text-sm leading-relaxed text-text-1/60">{announcement.body}</p>
                    <div className="mt-3">
      ...
      {activeTab === "assessments" ? (
        <StaffPanel>
          <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
            <div>
              <p className="eyebrow text-accent">Gradebook</p>
              <h3 className="mt-2 font-display text-xl text-text-1">Assessments</h3>
            </div>
            <Link
            ...
            <div className="space-y-2">
              {assessments.docs.map((item) => (
                <Link
                  ...
                  className="flex items-center justify-between gap-3 rounded-[var(--radius-md)] border border-edge-subtle px-4 py-3 transition-colors hover:border-accent/35"
                >
                  <span>
                    <span className="block text-sm font-semibold text-text-1">{item.title}</span>
                    <span className="text-xs text-text-3">
                      {item.kind} · {item.status}
                      {item.dueAt ? ` · due ${new Date(item.dueAt).toLocaleDateString("en-GH")}` : ""}
                    </span>
                  </span>
                  <span className="text-xs text-text-3">{item.totalMarks || 0} marks</span>
                </Link>
              ))}
            </div>
          ) : (
      ...
      {activeTab === "gradebook" ? (
        <StaffPanel>
          <h3 className="font-display text-xl text-text-1">Submitted work</h3>
          <p className="mt-2 mb-4 text-sm text-text-2">Queue of work waiting for a score or return.</p>
              ...
              {submissions.docs.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="text-xs text-accent">Grade</span>
                  </Link>
                );
              })}
            </div>
      ...
      {activeTab === "learners" ? (
        <StaffPanel>
          <h3 className="font-display text-xl text-text-1">Learners</h3>
          <p className="mt-2 text-sm text-text-2">
            {enrollments.totalDocs} enrolled · progress overrides stay under More tools.
          </p>
          {focusMember ? (
            <p className="mt-2 text-xs text-accent">Focused on member {focusMember}</p>
          ) : null}
            ...
            {enrollments.docs.slice(0, 25).map((item) => {
              ...
              return (
                <Link
                  ...
                  className={cn(
                    "flex items-center justify-between gap-3 rounded-xl px-3 py-3 transition hover:bg-inset",
                    focused && "border border-accent/35 bg-accent-bg",
                  )}
                >
                  <b className="text-sm text-text-1">{label}</b>
                  <span className="text-xs text-text-3">
          ...
          <details className="mt-6 rounded-[var(--radius-lg)] border border-edge-subtle bg-inset p-4">
            <summary className="cursor-pointer text-sm text-text-2">Progress override</summary>
            <div className="mt-4">
      ...
      {activeTab === "analytics" ? (
        <StaffPanel>
          <h3 className="font-display text-xl text-text-1">Analytics</h3>
          ...
          />
          <h4 className="mt-6 mb-3 text-sm font-medium text-text-1">Module drop-off</h4>
          {analytics.moduleStats.length ? (
            <div className="overflow-x-auto rounded-2xl border border-edge-subtle">
              <table className="min-w-full text-left text-sm">
                <thead className="bg-inset text-xs uppercase tracking-wider text-text-3">
                  <tr>
                ...
                <tbody>
                  {analytics.moduleStats.map((item) => (
                    <tr key={item.id}>
                      <td className="border-t border-edge-subtle px-4 py-3 text-text-2">{item.title}</td>
                      <td className="border-t border-edge-subtle px-4 py-3 text-text-2">{item.reached}</td>
                      <td className="border-t border-edge-subtle px-4 py-3 text-text-2">{item.completed}</td>
                      <td className="border-t border-edge-subtle px-4 py-3 text-text-2">{item.dropOff}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      {activeTab === "settings" ? (
        <StaffPanel>
          <h3 className="font-display text-xl text-text-1">Course settings</h3>
          ...
          {!readiness.ready ? (
            <div className="mb-5 rounded-[var(--radius-md)] border border-warn/30 bg-warn-bg px-4 py-3 text-sm text-warn" role="status">
              Not ready to publish. Missing: {readiness.missing.join(", ")}.
            </div>
          ) : (
            <div className="mb-5 rounded-[var(--radius-md)] border border-ai/30 bg-ai-bg px-4 py-3 text-sm text-ai" role="status">
      ...
      {activeTab === "settings" ? (
        <CourseDangerZone courseId={courseID} courseTitle={String(selected.title)} />
      ) : null}
    </div>
  );
}
