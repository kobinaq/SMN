        ...
        {media.docs.length ? (
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {media.docs.map((doc) => {
              ...
              return (
                <article key={doc.id} className="overflow-hidden rounded-[var(--radius-lg)] border border-edge-subtle bg-inset">
                  <div className="flex aspect-video items-center justify-center bg-inset">
                    {isImage && url ? (
                      ...
                      <span className="text-xs text-text-3">{doc.mimeType || "File"}</span>
                    )}
                  </div>
                  <div className="space-y-3 p-4">
                    <div>
                      <b className="block text-sm text-text-1">{doc.alt || "Untitled"}</b>
                      <small className="mt-1 block text-xs text-text-3">Attached via picker</small>
                    </div>
