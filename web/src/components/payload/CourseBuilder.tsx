...
export default async function CourseBuilder({ initPageResult, payload, searchParams }: AdminViewServerProps) {
  ...
  if (!selected) return <main className="smn-workspace"><span className="smn-eyebrow">Learning operations</span><h1>Course Builder</h1><p>Create the first course to begin building its curriculum.</p><Link className="smn-primary-action" href="/staff/learning/courses/new">Create course</Link></main>;
  ...
  return <main className="smn-workspace">
    <header className="smn-workspace-header"><div><span className="smn-eyebrow">Learning operations</span><h1>Course Builder</h1><p>Build, review, and operate a course without losing its relationships.</p></div><div className="smn-workspace-actions"><Link href={`/staff/learning?course=${courseID}&tab=settings`}>Advanced edit</Link><Link className="smn-primary-action" href="/staff/learning/courses/new">New course</Link></div></header>
    ...
    {activeTab === "curriculum" ? <section className="smn-curriculum"><div className="smn-section-heading"><div><span className="smn-eyebrow">Curriculum</span><h3>Modules and lessons</h3></div><Link href={`/staff/learning?course=${courseID}&tab=curriculum`}>Add module</Link></div>{modules.docs.map((courseModule, moduleIndex) => { const moduleLessons = lessonsByModule.get(String(courseModule.id)) ?? []; const lessonIDs = moduleLessons.map((item) => item.id); return <article key={courseModule.id}><header><div><small>Module {moduleIndex + 1}</small><h4>{courseModule.title}</h4></div><div><Link href={`/staff/learning/modules/${courseModule.id}`}>Edit</Link><ModuleActions courseId={courseID} moduleId={courseModule.id} moduleIds={moduleIDs} index={moduleIndex} empty={!moduleLessons.length} /></div></header><ol>{moduleLessons.map((lesson, lessonIndex) => <li key={lesson.id}><span>{lesson.title}<small>{lesson.lessonType} · {lesson.durationMinutes ?? 0} min</small></span><div><Link href={`/staff/learning/lessons/${lesson.id}`}>Edit</Link><LessonActions courseId={courseID} lessonId={lesson.id} lessonIds={lessonIDs} index={lessonIndex} moduleId={courseModule.id} modules={moduleOptions} /></div></li>)}</ol><Link href={`/staff/learning?course=${courseID}&tab=curriculum`}>Add lesson</Link></article>; })}</section> : null}
    {activeTab === "learners" ? <section className="smn-placeholder"><h3>Learners</h3><p>{enrollments.totalDocs} enrollment records grant access through program key <b>{selected.programKey}</b>.</p><Link href="/staff/learning">Manage enrollments in Learning</Link><h3>Reasoned progress override</h3><p>Every correction records the staff actor, timestamp, reason, and before/after state.</p><ProgressOverrideForm courseId={courseID} learners={learnerOptions} lessons={lessonOptions} /></section> : null}
    ...
    {activeTab === "settings" ? <section className="smn-placeholder"><h3>Settings</h3><p>Course access, publishing, certificate, and Tutor controls remain protected by the course record.</p><Link href={`/staff/learning?course=${courseID}&tab=settings`}>Edit course settings</Link></section> : null}
