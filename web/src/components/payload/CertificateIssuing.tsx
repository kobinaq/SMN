...
export default async function CertificateIssuing({ initPageResult, payload }: AdminViewServerProps) {
  ...
  return <main className="smn-workspace"><header className="smn-workspace-header"><div><span className="smn-eyebrow">Credentials</span><h1>Certificate Issuing</h1><p>Confirm eligibility, issue in bulk, prevent duplicates, and manage the credential lifecycle.</p></div><Link className="smn-primary-action" href="/staff/certificates">All certificates</Link></header><section className="smn-analytics-grid"><div><strong>{eligible.length}</strong><span>Eligible</span></div><div><strong>{valid.length}</strong><span>Valid certificates</span></div><div><strong>{certificates.docs.filter(item => item.status === "revoked").length}</strong><span>Revoked</span></div></section><section className="smn-ops-grid"><article><h2>1. Select eligible completions</h2>{eligible.length ? <CertificateIssuer eligible={eligible}/> : <p className="smn-empty">No eligible completions without a certificate.</p>}</article><article><h2>2. Review active credentials</h2>{valid.map(item => <div className="smn-ops-row" key={item.id}><div><Link href="/staff/certificates"><b>{item.title}</b></Link><span>{rel(item.member)?.name || "Member"} · {item.credentialCode} · notification {item.notificationStatus || "pending"}</span></div><CertificateLifecycle certificateId={item.id}/></div>)}</article></section></main>;
}
