...
export function MentorDirectory({ mentors }: { mentors: MentorDirectoryItem[] }) {
  ...
  return <>
    <div className="grid gap-3 sm:grid-cols-[1fr_220px]">
      ...
      <Select aria-label="Filter by topic" className={`${field} bg-surface`} value={topic} onChange={(event) => setTopic(event.target.value)}>{topics.map((item) => <option key={item}>{item}</option>)}</Select>
    </div>
