...
export function OpportunityApplyButton({ opportunityId }: { opportunityId: string | number }) {
  ...
  async function apply() {
    try {
      setState("opening");
      setError("");
      const json = await record("started");
      if (!json.applicationUrl) throw new Error("Application link is unavailable.");
      window.open(json.applicationUrl, "_blank", "noopener,noreferrer");
      setState("opened");
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : "Unable to open application.");
      setState("idle");
    }
  }
  ...
  return (
    <div className="flex flex-wrap items-center gap-3">
      <Button type="button" onClick={apply} disabled={state === "opening" || state === "saving"}>
        {state === "opening" ? "Opening…" : "Apply on employer site"}
        <ExternalLink className="h-4 w-4" />
      </Button>
      {state === "opened" || state === "saving" ? (
        <Button type="button" variant="secondary" onClick={markApplied} disabled={state === "saving"}>
          {state === "saving" ? "Saving…" : "Mark as applied"}
        </Button>
      ) : null}
      {state === "applied" ? (
        <span className="inline-flex items-center gap-1.5 text-sm text-ai">
