...
export function LearningDashboard({
  ...
  initialItems: LearningDashboardItem[];
  ...
  const programs = useMemo(
    () => ["All programs", ...Array.from(new Set(items.map((item) => item.programKey)))],
    [items],
  );
  ...
  const groups = Array.from(new Set(visible.map((item) => `${item.programKey}|${item.week}`))).map((key) => {
    const [programKey, week] = key.split("|");
    return {
      key,
      programKey,
      week: Number(week),
      items: visible.filter((item) => item.programKey === programKey && item.week === Number(week)),
    };
  });
      ...
      {enrollments.length ? (
        <section className="grid gap-4 md:grid-cols-2">
          {enrollments.map((enrollment) => {
            const isLive =
              Boolean(enrollment.classroomUrl) ||
              /cohort|live/i.test(enrollment.programType || "");
            return (
        ...
        <>
          <section className="rounded-2xl border border-white/10 bg-ink p-5">
            <div className="flex items-end justify-between gap-4">
              <div>
                <p className="text-xs uppercase tracking-[0.18em] text-baby-blue">Your progress</p>
                <p className="mt-2 font-display text-2xl text-white">
                  {completed} of {items.length} complete
                </p>
              </div>
              <strong className="text-3xl text-mint">{percentage}%</strong>
            </div>
          ...
          <div className="flex items-center justify-between gap-4">
            <h2 className="font-display text-2xl text-white">Learning plan</h2>
            {programs.length > 2 ? (
              <Select
                className="field max-w-[14rem] rounded-xl border border-white/10 bg-surface px-3 py-2 text-sm text-white"
                size="sm"
                value={program}
            ...
            {groups.map((group) => (
              <section key={group.key}>
                <div className="mb-3 flex items-center gap-3">
                  <span className="rounded-full bg-deep-blue px-3 py-1 text-xs text-white">
                    {group.week ? `Week ${group.week}` : "Start here"}
                  </span>
                  <span className="text-xs text-white/35">{group.programKey}</span>
                </div>
                <div className="space-y-2">
                  {group.items.map((item) => {
                    ...
                    return (
                      <article
                        key={item.id}
                        className="grid gap-4 rounded-2xl border border-white/10 bg-surface p-4 sm:grid-cols-[auto_1fr_auto] sm:items-center"
                      >
                        <button
                          type="button"
                          onClick={() => update(item.id, next)}
                          className={`flex h-10 w-10 items-center justify-center rounded-full border ${
                            item.status === "completed"
                              ? "border-mint/40 bg-mint/10 text-mint"
                              : "border-white/15 text-white/40"
                          }`}
                          aria-label={`Mark ${item.title} ${next}`}
                        >
                        ...
                        <div>
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            <span className="text-[10px] uppercase tracking-wide text-baby-blue">{item.kind}</span>
                          </div>
                          <p className="mt-1 text-sm leading-relaxed text-white/45">{item.summary}</p>
                          {item.estimatedMinutes ? (
                            <span className="mt-2 inline-flex items-center gap-1 text-xs text-white/30">
                        ...
                        {item.url ? (
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="inline-flex items-center gap-1 text-sm text-baby-blue"
                          >
